...
function MdTabsController ($scope, $element, $window, $mdConstant, $mdTabInkRipple, $mdUtil,
  ...
  function nextPage () {
    ...
    if (isRtl()) {
      ...
      for (i = 0; i < elements.tabs.length; i++) {
        ...
        if (totalTabWidths + tab.offsetWidth > totalWidth) break;

        totalTabWidths += tab.offsetWidth;
      }

      ctrl.offsetLeft = fixOffset(totalTabWidths);
    } else {
      for (i = 0; i < elements.tabs.length; i++) {
        tab = elements.tabs[ i ];
  ...
  function previousPage () {
    ...
    if (isRtl()) {
      var totalTabWidths = 0;
      var totalWidth = ctrl.offsetLeft - elements.canvas.clientWidth;
  ...
  function adjustOffset (index) {
    ...
    if (isRtl()) {
      ...
      ctrl.offsetLeft = Math.max(ctrl.offsetLeft, fixOffset(tabWidthsIncluding - elements.canvas.clientWidth + extraOffset));
    } else {
